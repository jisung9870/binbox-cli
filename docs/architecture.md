...
| `bb aws browse ...` / `bb aws query ...` | Opens a local-first progressive AWS TUI or runs a scoped query over EC2/AMI/IAM/Route 53 resources and relations, including exact AMI owner/visibility search across profiles | AWS CLI is limited to profile discovery/credential export; SDK STS verifies identity and narrowed SDK providers perform read calls; session state is memory-only and generation-fenced |
