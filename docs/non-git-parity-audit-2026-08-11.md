...
| `profile list/show/add/edit/rm/login` | migrated | AWS config; AWS CLI owns credentials/cache | `TestProfileManagesConfigOnlyAndPreservesFields` | restore the AWS config backup; authenticate again through `aws sso login` |
| `wenv list/current/set/rm/export/import` | migrated with a narrower declarative contract | bb XDG config | `TestWenvImportRejectsExecutableSyntaxAndExportsSafely`, byte-exact selector export assertion | re-import an allowlisted legacy preset; executable shell and secret-like variables stay retired |
