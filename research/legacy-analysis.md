...
| `tm` interactive selection, layouts, attach, kill | tmux-centric interaction and lifecycle | **Partially migrate** | Project open plus project/session inventory are Go commands; layouts, attach selection, and destructive bulk operations remain gated. |
...
| `gx`, `portcheck` | Git and local-port helpers | **Partially migrate** | Git root/branch/log inventory and non-destructive port inspection are typed Go commands; mutation remains separate. |
| `tfx` | Terraform workflow and apply safeguards | **Mostly migrate** | Core commands plus legacy-compatible session/apply/destroy are migrated; review/clean and state mutation remain gated. |
| `tvx` | Trivy scans and policy wrappers | **Migrate** | Direct Go adapter preserves CI/SBOM/report policy and confirmation-gates the node collector. |
| `kx`, `assume`, `assm` | Kubernetes and AWS helpers | **Defer by risk** | Add direct external adapters only with explicit mutation and credential boundaries. |
| `dx`, `md2jira` | Repo-configured Docker and unrelated local Python launchers | **Retire** | Do not embed checkout files or environment-specific project paths in the single binary. |
